package name.yumao.ffxiv.boot.swing;

import name.yumao.ffxiv.boot.util.FFXIVLauncher;
import name.yumao.ffxiv.boot.util.HexUtils;
import name.yumao.ffxiv.boot.util.res.Config;

import javax.swing.*;
import javax.swing.border.MatteBorder;
import java.awt.*;
import java.awt.event.*;
import java.io.File;

public class ClientLauncherPanel extends JFrame implements ActionListener {

    private static Point origin = new Point();
    private static String title = "提莫苑|快速启动";
    private JLabel title_lable = new JLabel(title);
    private Dimension dimension;
    private JButton closeButton = new JButton("x");
    private JPanel titlePanel = new JPanel();
    private JPanel bodyPanel = new JPanel();

    private JLabel userLable = new JLabel("账户");
    private JTextField userField = new JTextField();
    private JLabel pwdLable = new JLabel("密码");
    private JPasswordField pwdField = new JPasswordField();
    private JLabel otpLable = new JLabel("密保");
    private JPasswordField otpField = new JPasswordField();

    private JButton submitButton = new JButton("启动");
    private JButton exitButton = new JButton("关闭");

    public ClientLauncherPanel() {
        super("提莫苑|快速启动");
        setUndecorated(true);
        Toolkit toolkit = Toolkit.getDefaultToolkit();
        setIconImage(toolkit.createImage(HexUtils.hexStringToBytes("")));
        Dimension dimension=toolkit.getScreenSize();
        setBounds((dimension.width-230)/2, (dimension.height-165)/2, 230, 165);
        setResizable(false);
        setLayout(null);
        setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);

        titlePanel.setBounds(0, 0, 230, 30);
        titlePanel.setBackground(new Color(110,110,110));
        titlePanel.setBorder(new MatteBorder(0, 0, 0, 0, new Color(110,110,110)));
        add(titlePanel);
        bodyPanel.setBounds(0, 0, 230, 165);
        bodyPanel.setBackground(new Color(255,255,255));
        bodyPanel.setBorder(new MatteBorder(0, 1, 1, 1, new Color(110,110,110)));
        add(bodyPanel);
        title_lable.setBounds(10, 0, 150, 30);
        title_lable.setFont(new Font("Microsoft Yahei",Font.BOLD,13));
        title_lable.setForeground(new Color(255,255,255));
        add(title_lable,0);
        closeButton.setBounds(200, 0, 20, 30);
        closeButton.setFont(new Font("Microsoft Yahei",Font.BOLD,12));
        closeButton.setForeground(new Color(255,255,255));
        closeButton.setMargin(new Insets(0, 0, 0, 0));
        closeButton.setBorder(null);
        closeButton.setOpaque(false);
        closeButton.setIconTextGap(0);
        closeButton.setContentAreaFilled(false);
        closeButton.setFocusable(false);
        closeButton.addActionListener(this);
        add(closeButton,0);

        addMouseListener(new MouseAdapter() {
            public void mousePressed(MouseEvent e) {
                origin.x = e.getX();
                origin.y = e.getY();
            }
        });
        addMouseMotionListener(new MouseMotionAdapter() {
            public void mouseDragged(MouseEvent e) {
                Point p = getLocation();
                setLocation(p.x + e.getX() - origin.x, p.y + e.getY()- origin.y);
            }
        });
        addWindowListener( new WindowAdapter() {
            public void windowOpened( WindowEvent e ){
                otpField.requestFocus();
            }
        });

        userLable.setBounds(30, 40, 100, 25);
        userLable.setFont(new Font("Microsoft Yahei",Font.BOLD,13));
        userLable.setForeground(new Color(110,110,110));
        add(userLable,0);
        userField.setBounds(80, 42, 120, 20);
        userField.setFont(new Font("Microsoft Yahei",Font.BOLD,12));
        userField.setForeground(new Color(110,110,110));
        add(userField,0);
        pwdLable.setBounds(30, 70, 100, 25);
        pwdLable.setFont(new Font("Microsoft Yahei",Font.BOLD,13));
        pwdLable.setForeground(new Color(110,110,110));
        add(pwdLable,0);
        pwdField.setBounds(80, 72, 120, 20);
        pwdField.setFont(new Font("Microsoft Yahei",Font.BOLD,12));
        pwdField.setForeground(new Color(110,110,110));
        add(pwdField,0);
        otpLable.setBounds(30, 100, 100, 25);
        otpLable.setFont(new Font("Microsoft Yahei",Font.BOLD,13));
        otpLable.setForeground(new Color(110,110,110));
        add(otpLable,0);
        otpField.setBounds(80, 102, 120, 20);
        otpField.setFont(new Font("Microsoft Yahei",Font.BOLD,12));
        otpField.setForeground(new Color(110,110,110));
        add(otpField,0);


        rootPane.setDefaultButton(submitButton);
        submitButton.setBounds(30, 130, 60, 25);
        submitButton.setFont(new Font("Microsoft Yahei",Font.BOLD,12));
        submitButton.setForeground(new Color(110,110,110));
        submitButton.setMargin(new Insets(0, 0, 0, 0));
        submitButton.setOpaque(false);
        submitButton.setIconTextGap(0);
        submitButton.setContentAreaFilled(false);
        submitButton.setFocusable(false);
        submitButton.addActionListener(this);
        add(submitButton,0);
        exitButton.setBounds(140, 130, 60, 25);
        exitButton.setFont(new Font("Microsoft Yahei",Font.BOLD,12));
        exitButton.setForeground(new Color(110,110,110));
        exitButton.setMargin(new Insets(0, 0, 0, 0));
        exitButton.setOpaque(false);
        exitButton.setIconTextGap(0);
        exitButton.setContentAreaFilled(false);
        exitButton.setFocusable(false);
        exitButton.addActionListener(this);
        add(exitButton,0);



        if (Config.getProperty("Username") != null &&Config.getProperty("Username").length() > 0) {
            userField.setText(Config.getProperty("Username"));
        }
        if (Config.getProperty("Password") != null &&Config.getProperty("Password").length() > 0) {
            pwdField.setText(Config.getProperty("Password"));
        }

        setVisible(false);
        setVisible(true);
    }

    @Override
    public void actionPerformed(ActionEvent e) {
        if(e.getSource() == closeButton || e.getSource() == exitButton){
            System.exit(0);
        }
        if (e.getSource() == submitButton){
            Config.setProperty( "Username", userField.getText());
            Config.setProperty( "Password", new String(pwdField.getPassword()));
            Config.saveProperty();

            String launchParams = FFXIVLauncher.getlaunchParams(userField.getText(), new String(pwdField.getPassword()), new String(otpField.getPassword()));
            String params[] = launchParams.split(",");
            if (params[1].equals("err")) {
                JOptionPane.showMessageDialog(null, params[2], "登陆错误",JOptionPane.ERROR_MESSAGE);
            } else {
                String sid = FFXIVLauncher.getSid(launchParams).split(",")[0];
                String scode = FFXIVLauncher.getSid(launchParams).split(",")[1];
                if (!scode.equals("204")) {
                    JOptionPane.showMessageDialog(null, "<html><body>检测到客户端有更新内容<br />请使用官方登陆器进行登陆以更新客户端</body></html>", "需要更新",JOptionPane.ERROR_MESSAGE);
                    this.dispose();
                } else {
                    FFXIVLauncher.launchClient(launchParams, sid);
                    System.exit(0);
                }
            }
        }
    }

    public static void main(String[] args) {
        new ClientLauncherPanel();
    }
}
